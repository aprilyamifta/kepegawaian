<?php if ($this->session->userdata('logged_in') == true) : ?>
    ...
    <footer class="py-4 bg-light mt-auto">
        <div class="container-fluid">
            <div class="d-flex align-items-center justify-content-between small">
                <div class="text-muted">Copyright &copy; <?= date("Y"); ?><a href="<?= base_url(); ?>" class="ml-1"><?= $appname = (empty($dataapp['nama_app_absensi'])) ? 'Absensi Online' : $dataapp['nama_app_absensi']; ?> SMPN10 PLG</a>
                    <div class="d-inline">Powered By<a href="" class="ml-1">WGS-lab</a></div>
                </div>
    ...
    <script>
        $("#refresh-tabel-kinerja").click(function(e) {
            e.preventDefault();
            load_process();
            $('#listkinerjaku').DataTable().ajax.reload();
        });

        $('#listkinerjaku').DataTable({
            "ajax": {
                url: "<?= base_url('ajax/get_datatbl?type=allself'); ?>",
                type: 'get',
                async: true,
